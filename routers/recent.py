...
async def fetch_recent_for_site(site, category, page, limit, all_sites):
    if site in all_sites and all_sites[site]["recent_available"]:
        ...
        if category and category not in all_sites[site]["categories"]:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Selected category not available.")
        return await all_sites[site]["website"]().recent(category, page, limit)
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Recent search not available for {site}.")

async def fetch_recent_from_all_sites(limit, all_sites, start_time):
    sites_list = [site for site, config in all_sites.items() if config["recent_available"]]
    tasks = [asyncio.create_task(all_sites[site]["website"]().recent(None, 1, limit)) for site in sites_list]
    ...
    if COMBO["total"] == 0:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No recent torrents found.")
